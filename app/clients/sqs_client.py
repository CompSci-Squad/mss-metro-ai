    ...
    for attempt in range(max_retries):
        try:
            async with aioboto3.client("sqs", endpoint_url=settings.aws_endpoint) as sqs:
                await sqs.send_message(QueueUrl=settings.sqs_queue_url, MessageBody=json.dumps(message))
                logger.info("message_enqueued", queue=settings.sqs_queue_url, attempt=attempt + 1)
                return
        except Exception as e:
            last_error = e
            logger.warning("sqs_send_failed", attempt=attempt + 1, max_retries=max_retries, error=str(e))
